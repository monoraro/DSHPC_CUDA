...
replica = archivos_ordenados[100]
...
max_position_gpu = gpuarray.zeros((U.shape[1],), dtype=np.int32)

# Ejecutar el kernel de búsqueda binaria
coordenadas_maximo(U_gpu, np.int32(U.shape[1]), np.int32(U.shape[0]), max_position_gpu, block=(block_size, 1, 1), grid=(grid_size, 1))
...
mascara = crear_mascara_circular(U.shape,(row_index,col_index),200)
cuadrante_gpu = gpuarray.to_gpu(mascara.astype(np.float32))
...
def update(val):
    ...
    ax.clear()
    ax.imshow(video_array[frame].astype(np.unit8))
...
np.savetxt('tiempos_paralelos.txt', tiempo_frame, fmt='%f', delimiter='\t')
...
video_path = 'Video_40x_paralellized.mp4'  # Puedes cambiar la extensión según el formato deseado (mp4, gif, etc.)
